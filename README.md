...
Criar um programa em Python que permita ao departamento de manutenção de equipamentos informáticos gerir pedidos de forma eficiente, utilizando funções para modularidade e organização do código.
"""
def registar_pedido(pedidos): #função para registar pedido 
...
def consultar_pedido(pedidos): #função para consultar pedido
...
def atualizar_estado(pedidos): #função para atualizar estado do pedido
...
def eliminar_pedido(pedidos): #função para eliminar pedido
...
def exibir_pedidos(pedidos): #função para exibir pedidos
    print("\nLista de Pedidos:")
